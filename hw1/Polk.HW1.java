...
public class HW1 {
    ...
    static class LinkedList {
        ...
        public void removeElement ( int value ) {
            // if there is no LinkedList there is nothing to remove
            if (head == null) {
                return;
            }

            // if the value is at the front of the list updating head forward effectively removes it
            while (head != null && head.data == value) {
                head = head.next;
            }

            // removing elements in the middle by circumventing their node
            Node curr = head;
    ...
    static class Stacks {
        ...
        public static boolean isPalindrome(String input) {
            ...
            // removes all whitespace via regex
            input = input.toLowerCase().replaceAll("\\s+", "");

            int l = 0;
            int r = input.length() - 1;
            // two pointers moving toward eachother. For a palindrome this should see if any character
            // is different forwards from backwards
            while (l < r) {
        ...
        public static int findLargestK(Stack<Integer> stack, int k) {
            ...
            int tempIndex = 0; // keeps track of current index from top to bottom
            Stack<Integer> tempStack = new Stack<>();

            int size = stack.size(); // size is dynamic so it needs to be stored
            while (!stack.isEmpty()) {
                int value = stack.pop();
                tempStack.push(value);
                /*
                 * to calculate current index from the bottom, I take the total size of the stack subtracted from
                 * the iteration count subtracted by 1 for 0 indexing to get the true index from the bottom
                 * if we know the size, and we know where we are from the front, its difference will be the index from
                 * the bottom. The size is 1 more than the index so we need to account for that as well.
                 */
                if (value == k && size - tempIndex - 1 > largestIndex) {
                    largestIndex = size - tempIndex - 1;
                }
                tempIndex += 1;
            }
