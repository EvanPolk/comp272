...
public class BinaryTree {
    ...
    private int findMinHelper(Node node) {
        ...
        int lowerMin = Math.min(findMinHelper(node.left), findMinHelper(node.right));

        return Math.min(node.data, lowerMin);
    }
    ...
    private int nodesGTHelper(Node node, int val) {
        ...
        if (node.data > val) {
            return nodesGTHelper(node.left, val) + nodesGTHelper(node.right, val) + 1;
        }
